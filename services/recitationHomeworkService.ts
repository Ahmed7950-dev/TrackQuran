...
export const rangeLabel = (r: Pick<RecitationHomework, 'startSurah' | 'startAyah' | 'endSurah' | 'endAyah'>): string => {
...
export async function submitRecitationHomework(rec: RecitationHomework): Promise<RecitationHomework | null> {
  ...
  const verses = versesOfRange(done).length;
...
export async function reviewRecitationHomework(
  ...
  await createNotification({
    ...
    title: verdict === 'passed' ? '✅ Recitation homework passed' : '🔁 Recitation needs revision',
    body: verdict === 'passed'
      ? `Well done! Your teacher reviewed ${rangeLabel(done)}. Tap to see any notes.`
      : `Your teacher left notes on ${rangeLabel(done)}. Tap to see the mistakes, then record again.`,
    metadata: {
