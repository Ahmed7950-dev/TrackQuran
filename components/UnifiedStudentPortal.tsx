...
const UnifiedStudentPortal: React.FC<{ token: string }> = ({ token }) => {
  ...
  // After choosing, the selected portal renders full-screen with its OWN header;
  // the switch control is injected into that header (no separate top bar here).
  if (view === 'arabic') {
    return (
      <ArabicStudentPortal
        token={pair.arabicShareToken}
  ...
  if (view === 'quran') {
    return (
      <SharedReportPage
        reportId={pair.quranReportId}
        switchPortal={{ label: 'Arabic', onSwitch: () => setView('arabic') }}
      />
    );
  }

  // view === 'choose' — the two-option landing screen.
  return (
