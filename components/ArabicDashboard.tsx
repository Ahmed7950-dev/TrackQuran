...
import { familyHeading } from './Dashboard';
...
const ArabicDashboard: React.FC<Props> = ({
  ...
  onAddStudent, onSelectStudent, onUpdateStudent, onFamilyLinks, familyGroups,
  archivedIds = [],
  ...
  // Same as the Quran dashboard: siblings sit the same lesson, so the card pins
  // the whole family. Only members on this roster resolve here.
  ...
  // ── Meet link ─────────────────────────────────────────────────────────────
  /** The sessions this lesson covers: its own, or every family member's session
   *  at the same time — siblings share one room, so they share one link. */
  function familySessionIds(): string[] {
    if (!nextLesson?.sessionId) return [];
    const members = lessonFamily?.members.map(m => m.id);
    if (!members) return [nextLesson.sessionId];
    const at = nextLesson.date.getTime();
    return sessions
      .filter(s => members.includes(s.studentId)
        && Math.abs(new Date(s.startAt).getTime() - at) < 60_000)
      .map(s => s.id);
  }
  ...
  async function handleClearMeetLink() {
    ...
    if (ids.length > 0) {
      ...
      setSessions(prev => prev.map(s => ids.includes(s.id) ? { ...s, meetUrl: undefined } : s));
    } else if (nextLesson.bookingId) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── Next Lesson card ─────────────────────────────────────────────────
          Same as the Quran dashboard: one student's card opens them, a family's
          card is inert and only its avatars and Meet controls act. ── */}
      {nextLesson && (() => {
        ...
        const avatar = (member: ArabicStudent, withName: boolean) => (
          <button
            key={member.id}
            type="button"
        ...
        return (
          <div
            ...
            className={`group rounded-2xl border overflow-hidden cursor-pointer transition-all duration-200 hover:shadow-xl hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-900 ${cardTone}`}
          >
