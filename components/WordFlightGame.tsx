  ...
  <div className="grid grid-cols-3 gap-2">
    {PLANES.map((p, i) => (
      <button key={i} onClick={() => onSelect(i)}
        ...
        style={{ borderColor: selected === i ? accentColor : '#e2e8f0', background: selected === i ? `${accentColor}18` : 'white', transform: selected === i ? 'scale(1.06)' : 'scale(1)', boxShadow: selected === i ? `0 4px 12px ${accentColor}44` : undefined }}>
...
const WordFlightGame: React.FC<WordFlightGameProps> = ({ words, onExit, roomId: propRoomId, playerRole }) => {
  ...
  const overlay   = (children: React.ReactNode) => (
    <div className="absolute inset-0 z-30 flex items-center justify-center" style={{ background:'rgba(10,30,80,0.55)', backdropFilter:'blur(4px)' }}>
      <div className="bg-white rounded-3xl shadow-2xl border-4 border-sky-200 px-6 py-6 text-center max-w-sm mx-4 w-full">{children}</div>
    </div>
  );
    ...
    return (
      <div className="select-none" style={{ position:'fixed', inset:0, zIndex:50, background:'#1a6fc4', touchAction:'none' }}>
        ...
        {!p2Waiting && status === 'start' && overlay(
          <>
            <div className="inline-block px-3 py-1 rounded-full text-xs font-extrabold text-white mb-3" style={{ background:'#f97316' }}>Word Flight — Player 2</div>
            <div className="mb-3">
  ...
  return (
    <div className="select-none" style={{ position:'fixed', inset:0, zIndex:50, background:'#1a6fc4', touchAction:'none' }}>
      ...
      {status === 'start' && overlay(
        <>
          <h3 className="text-xl font-extrabold text-sky-700 mb-3">Word Flight! 📖✈️</h3>
          ...
          <p className="text-xs font-bold text-indigo-500 mb-4">
            {gameMode==='2p-online'?'⬆️⬇️⬅️➡️ You · Space=fire · P2 uses their device'
              ...
              :'⬆️⬇️⬅️➡️ to fly · Space=fire powerup · Hear the word, find it!'}
          </p>
          <p className="text-xs text-slate-400 mb-3">{words.length} words loaded</p>
          <div className="flex gap-2 justify-center">
