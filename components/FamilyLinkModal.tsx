...
import {
  getStudentReportId, createOrUpdateSharedReport,
...
const EMPTY_FORM: Omit<FamilyLink, 'id' | 'teacher_id' | 'created_at' | 'updated_at'> = {
  name: '',
  members: [],
};
...
const FamilyLinkModal: React.FC<Props> = ({
  ...
  const toggleMember = (type: 'quran' | 'arabic', student: Student | ArabicStudent) => {
    const key = type === 'quran'
      ? `quran-${(student as Student).id}`
      : `arabic-${(student as ArabicStudent).id}`;
    const existing = form.members.find(m => `${m.type}-${m.report_id ?? m.share_token ?? ''}`.includes(
      type === 'quran' ? (student as Student).id : (student as ArabicStudent).id
    ));
    if (existing) {
      setForm(f => ({ ...f, members: f.members.filter(m => m !== existing) }));
    } else {
      const newMember: FamilyMember = {
        id: crypto.randomUUID(),
        name: student.name,
        type,
        report_id: type === 'quran' ? undefined : undefined,
        share_token: type === 'arabic' ? (student as ArabicStudent).shareToken : undefined,
      };
  ...
  const isMemberSelected = (type: 'quran' | 'arabic', id: string) =>
    form.members.some(m =>
      m.type === type && (
        type === 'quran'
          ? (m as any)._studentId === id
          : m.share_token === arabicStudents.find(s => s.id === id)?.shareToken
      )
    );

  // ── Simpler toggle using student ID directly ───────────────────────────────
  ...
  const openEdit = (link: FamilyLink) => {
    ...
    setForm({ name: link.name, members: link.members });
    const qIds = new Set(link.members.filter(m => m.type === 'quran').map(m => (m as any)._studentId ?? ''));
    const aIds = new Set(link.members.filter(m => m.type === 'arabic').map(m =>
      arabicStudents.find(s => s.shareToken === m.share_token)?.id ?? ''
    ).filter(Boolean));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      for (const sid of Array.from(selectedQuranIds)) {
        ...
        if (!reportId) { console.warn('Could not create report for', student.name); continue; }
        members.push({ id: crypto.randomUUID(), name: student.name, type: 'quran', report_id: reportId });
      }

      // Process Arabic students
      ...
      await saveFamilyLink({ id, teacher_id: teacherId, name: form.name.trim(), members });
      setShowForm(false);
      reload();
    } catch (err) {
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
              ...
              {links.map(link => (
                <div key={link.id} className="bg-slate-50 dark:bg-gray-700 rounded-xl border border-slate-200 dark:border-gray-600 p-4">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                        {link.members.length} {link.members.length === 1 ? 'child' : 'children'} ·{' '}
                        {link.members.filter(m => m.type === 'quran').length > 0 && `${link.members.filter(m => m.type === 'quran').length} Quran`}
                        {link.members.filter(m => m.type === 'quran').length > 0 && link.members.filter(m => m.type === 'arabic').length > 0 && ', '}
                        {link.members.filter(m => m.type === 'arabic').length > 0 && `${link.members.filter(m => m.type === 'arabic').length} Arabic`}
                      </p>
                      <div className="flex flex-wrap gap-1 mt-1.5">
                        {link.members.map(m => (
                          <span key={m.id} className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                            m.type === 'quran' ? 'bg-teal-100 dark:bg-teal-900/40 text-teal-700 dark:text-teal-300' : 'bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300'
                          }`}>
                            {m.name}
                          </span>
                        ))}
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      <button onClick={() => openEdit(link)} className="p-1.5 rounded-lg hover:bg-slate-200 dark:hover:bg-gray-600 text-slate-500 dark:text-slate-400 transition-colors" title="Edit">
                      ...
                      {deleteConfirmId === link.id ? (
                        ...
                        <button onClick={() => setDeleteConfirmId(link.id)} className="p-1.5 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/30 text-slate-500 dark:text-slate-400 hover:text-red-600 dark:hover:text-red-400 transition-colors" title="Delete">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
                  ...
                  <button
                    ...
                    className={`mt-2 w-full flex items-center justify-center gap-2 py-2 rounded-xl text-xs font-semibold border transition-all ${
                      copiedId === link.id
          ...
          {showForm && (
            <div className="space-y-4">
              <button onClick={() => setShowForm(false)} className="flex items-center gap-1.5 text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4">
              ...
              {/* Family name */}
              <div>
                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">Family name</label>
                <input
                  value={form.name}
                  onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                  placeholder="e.g. Al-Hassan Family"
                  className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 dark:text-slate-100"
                />
              </div>
                        ...
                        <button
                          ...
                          className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl border text-left transition-all ${
                            checked
                              ...
                              : 'bg-white dark:bg-gray-700 border-slate-200 dark:border-gray-600 hover:border-teal-200 dark:hover:border-teal-700'
                          }`}
                        >
                          <div className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 ${
                            checked ? 'bg-teal-600 border-teal-600' : 'border-slate-300 dark:border-gray-500'
                          }`}>
                        ...
                        <button
                          ...
                          className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl border text-left transition-all ${
                            checked
                              ...
                              : 'bg-white dark:bg-gray-700 border-slate-200 dark:border-gray-600 hover:border-amber-200 dark:hover:border-amber-700'
                          }`}
                        >
                          <div className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 ${
                            checked ? 'bg-amber-500 border-amber-500' : 'border-slate-300 dark:border-gray-500'
                          }`}>
                            {checked && <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={3} stroke="white" className="w-3 h-3"><path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" /></svg>}
                          </div>
        ...
        {showForm && (
          <div className="px-6 py-4 border-t border-slate-200 dark:border-gray-700 flex-shrink-0 flex gap-3">
            ...
            <button
              onClick={handleSave}
              disabled={saving || !form.name.trim() || (selectedQuranIds.size === 0 && selectedArabicIds.size === 0)}
